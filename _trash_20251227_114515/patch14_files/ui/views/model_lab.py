...
def _load_latest(conn: sqlite3.Connection, cfg_hash: Optional[str]) -> pd.DataFrame:
    """Load latest picks + model scores.

    Important: picks_daily/model_scores_daily can contain multiple runs for the same trade_date.
    We must bind the UI to the *latest run_id* (newest created_at) to avoid mixing results.
    """

    # latest run (by trade_date then created_at)
    row = conn.execute(
        """SELECT trade_date, run_id, config_hash
             FROM picks_daily
    ...
    latest_cfg_hash = str(row["config_hash"])

    # If user explicitly picked a config hash, bind to its latest run_id on the same trade_date.
    if cfg_hash and cfg_hash != latest_cfg_hash:
        r2 = conn.execute(
            """SELECT run_id
                 FROM picks_daily
        ...
        if r2 and r2[0]:
            latest_run_id = str(r2[0])
        else:
            # fallback to latest
            cfg_hash = latest_cfg_hash
    else:
        cfg_hash = latest_cfg_hash

    rank_col = safe_rank_column(conn)

    # picks (latest run only)
    picks = pd.read_sql_query(
        f"""SELECT p.trade_date, p.ts_code, p.name, p.industry,
                  ...
                  p.config_hash, p.run_id
             FROM picks_daily p
             WHERE p.trade_date=? AND p.run_id=? AND p.config_hash=?
             ORDER BY {rank_col} ASC
        """,
        conn,
        params=(trade_date, latest_run_id, cfg_hash),
    )
    ...
    ms = pd.read_sql_query(
        """SELECT trade_date, ts_code,
             ...
             WHERE trade_date=? AND config_hash=? AND run_id=?
        """,
        conn,
        params=(trade_date, cfg_hash, latest_run_id),
    )
...
def render(cfg_path: str = "config/config.yaml") -> None:
    ...
    def render_card(r: pd.Series, prefix: str = "") -> None:
        ...
        ds_comment = r.get("comment_ds") or "（未启用模型：当前走纯规则）"
        qw_comment = r.get("comment_qw") or "（未启用模型：当前走纯规则）"
