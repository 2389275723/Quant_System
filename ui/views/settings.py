...
from src.core.env import load_env_from_root
...
def render(cfg_path: str) -> None:
    ...
    # Make .env effective immediately (no restart required)
    try:
    ...
    with col2:
        ...
        deepseek_model = st.selectbox(
            ...
            index=0 if str(get(cfg, "model.deepseek_model", "deepseek-chat")) == "deepseek-chat" else 1,
            help="deepseek-chat = 非思考版本；deepseek-reasoner = 思考(推理)版本",
        )
        qwen_model = st.text_input(
            "Qwen 模型名",
            value=str(get(cfg, "model.qwen_model", "qwen3-max")),
            help="DashScope OpenAI 兼容接口的模型名，例如 qwen3-max / qwen-plus / qwen-turbo 等",
        )

        rerank_enabled = st.checkbox(
            "启用 AI 二次排序（TopN 重新排序）",
            value=bool(get(cfg, "model.rerank_enabled", True)),
            help="会在规则选出的 TopM 内，用 AI 输出(Alpha/Risk)合成打分后重新排序。",
        )
        news_enabled = st.checkbox(
            "启用资讯快照（行业资金流/涨停概念/GDELT 快讯）",
            value=bool(get(cfg, "news.enabled", True)),
            help="免费快讯来自 GDELT；行业资金流/概念涨停需网关支持对应 Tushare 接口。",
        )
        news_hours = st.number_input(
            "快讯时间窗（小时）",
            min_value=1,
            max_value=72,
            value=int(get(cfg, "news.gdelt.hours", 8) or 8),
            step=1,
        )
        news_limit = st.number_input(
            ...
            min_value=5,
            max_value=50,
            value=int(get(cfg, "news.gdelt.limit", 12) or 12),
            step=1,
        )
    ...
    if st.button("💾 保存设置", width='stretch'):
        ...
        # Make it effective immediately (no Streamlit restart required)
        try:
            load_env_from_root(root, override=True)
        except Exception:
            pass
        ...
        cfg2["model"]["deepseek_model"] = str(deepseek_model)
        cfg2["model"]["qwen_model"] = str(qwen_model)
        cfg2["model"]["rerank_enabled"] = bool(rerank_enabled)

        cfg2.setdefault("news", {})
        cfg2["news"]["enabled"] = bool(news_enabled)
        cfg2["news"].setdefault("gdelt", {})
        cfg2["news"]["gdelt"]["hours"] = int(news_hours)
        cfg2["news"]["gdelt"]["limit"] = int(news_limit)

        cfg2.setdefault("ui", {})
        if trade_date:
            cfg2["ui"]["trade_date"] = trade_date

        if allow_growth_boards:
